...
### Step 5 — Pick PR base (detect stacking) and run `gh pr create`

Before composing the `gh pr create` command, detect whether this wave was built atop a previous wave whose commits aren't yet on `<upstream>/<upstream-default-branch>`. Run:

```
git fetch <upstream> <upstream-default-branch>
git log --format="%H %s" <upstream>/<upstream-default-branch>..<wave-branch>
```

Cross-reference the output with the spec's `_Wave N executed ..._` annotations and any prior `swarm/*` branches that still exist locally. If commits from a previous wave appear in the range (i.e. the wave was stacked on a previous wave's branch in Phase 1.5 / 3.5), `AskUserQuestion`:

- **Stack — `--base <previous-wave-branch>`** — clean diff (only this wave's commits). Creates a stacked PR. After it merges, a follow-up `<previous-wave-branch> → <upstream-default-branch>` PR is required to land the work on the default branch. Phase 10 will record `landed_on_main: no; trailing_pr: pending` so Phase 1a's landing gate catches it next run.
- **Direct — `--base <upstream-default-branch>`** — diff includes the previous wave's commits. Acceptable only if the previous wave's PR is in flight and expected to merge before this one (GitHub will rebase the diff once upstream merges). Surface the risk before the user picks this.
- **Wait** — abort PR creation. Tell the user to merge the previous wave's PR first, then re-run `/swarm` from Phase 8.5.

If no stacking is detected (the wave is built fully atop `<upstream>/<upstream-default-branch>`), use `--base <upstream-default-branch>` without prompting.

Show the user the composed title, body, and exact command with the chosen base. Get explicit confirmation. Then run:
...
Capture the PR URL from `gh`'s output for the wave annotation in Phase 10. Also remember the chosen base — Phase 10's landing check will rely on it to decide whether to mark `landed_on_main: no; trailing_pr: pending`.
